...
class Log1PLoss(Metric):
    """Log1P loss.

    Log1P loss penalty is the sum of the Log1Ps of one plus the log1p values of the learned
    activations, averaged over the number of activation vectors.

    Example:
        >>> Log1P_loss = Log1PLoss()
        >>> learned_activations = torch.tensor([
        ...     [ # Batch 1
        ...         [1., 0., 1.] # Component 1: learned features (Log1P of 2)
        ...     ],
        ...     [ # Batch 2
        ...         [0., 1., 0.] # Component 1: learned features (Log1P of 1)
        ...     ]
        ... ])
        >>> Log1P_loss.forward(learned_activations=learned_activations)
        tensor(1.0397)
    """
    ...
    def keep_batch_dim(self, keep_batch_dim: bool) -> None:
        ...
        if keep_batch_dim and not isinstance(self.log1p_loss, list):
            self.add_state(
                "log1p_loss",
                default=[],
                dist_reduce_fx="sum",
            )
        elif not isinstance(self.log1p_loss, Tensor):
            self.add_state(
                "log1p_loss",
    ...
    @staticmethod
    def calculate_abs_log1p_sum(
        learned_activations: Float[
            Tensor, Axis.names(Axis.BATCH, Axis.COMPONENT_OPTIONAL, Axis.LEARNT_FEATURE)
        ],
    ) -> Float[Tensor, Axis.names(Axis.BATCH, Axis.COMPONENT_OPTIONAL)]:
        """Calculate the absolute log1p sum of the learned activations.

        Args:
            learned_activations: Learned activations (intermediate activations in the autoencoder).

        Returns:
            absolute log1p sum of the learned activations (keeping the batch and component axis).
        """
        return torch.abs(learned_activations).log1p().sum(dim=-1)
    ...
    def update(
        ...
        **kwargs: Any,  # type: ignore # noqa: ARG002, ANN401 (allows combining with other metrics)
        ...
        If we're keeping the batch dimension, we simply take the log1p sum of the activations
        (over the features dimension) and then append this tensor to a list. Then during compute we
        just concatenate and return this list. This is useful for e.g. getting Log1P loss by batch
        ...
        """
        log1p_loss = self.calculate_abs_log1p_sum(learned_activations)
    ...
    def compute(self) -> Tensor:
        """Compute the metric."""
        return (
            torch.cat(self.log1p_loss)  # type: ignore
            if self.keep_batch_dim
            else self.log1p_loss / self.num_activation_vectors
        )
